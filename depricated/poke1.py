...
from util import UtilityFactory
...
if os.path.isfile(HOME + "/.poke/global") is False:
	HEADER = open(HOME + '/.poke/global', 'wb+')
	print("Global file not found!")
	HEADER.write("Init Application on " + time.strftime("%c"))
	HEADER.write("$HOME " + HOME)
	HEADER.close()
else:
	HEADER = open(HOME + "/.poke/global", "w")
...
if(serverSetup and keySetup):
	print("Run 'poke -?' to get into vi and edit your server and key settings.")
	sys.exit()
...
utilFactory = UtilityFactory(HOME, configReader)
...
parser.add_option("-K", action="store", help="Overwrite stored key-setting for a server", type="string", dest="SSHKEY")
...
for server in servers:
	...
	if 'xdef' in ConfigSectionmMap(server):
		...
		xdef = "'False'"
	if 'key' in ConfigSectionmMap(server):
		key = "'%s'" % ConfigSectionmMap(server)['key']
	else:
		key = "'Null'"
